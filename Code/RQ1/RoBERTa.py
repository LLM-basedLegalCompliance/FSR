...
#df = load_data('/home/shabnam/Test/Annotation.csv')
...
bert_tok = RobertaTokenizer.from_pretrained(config.model_name)
...
def train_on_full_data(params):
  ...
  for epoch in range(n_epochs):
      ...
      for i, batch in enumerate(full_train_loader):
          batch = tuple(t.cuda() for t in batch)
          input_ids, input_mask, label_ids = batch #segment_ids
          outputs = model(input_ids, attention_mask=input_mask, labels=label_ids)#token_type_ids=segment_ids
          loss = outputs[0]
          logits = outputs[1]
          optimizer.zero_grad()
          loss.backward()
